...
int main(int argc, char *argv[])
{
	...
	init_cards();
	start_game(8, 0, cards, 100, 10, 10);
...
int init_card_background()
{
	...
	{
		card_background[i].load_picture(c);
...
int init_cards()
{
	...
	for (int i = 0; i < 1000; i++)
	{
		cards[i].picture = (Picture *)malloc(sizeof(Picture));
	...
	{
		(*cards[j].picture).load_picture(c);
